...
document.addEventListener('DOMContentLoaded', function() {
	...
	const secondToLast = path.slice(-2, -1)
	const onStep1 = Number.isNaN(parseInt(secondToLast[0])) && secondToLast[0] !== 'end'

	if(score.get() === null && !onStep1) {
		const newPath = [...path.slice(0, -2), ...path.slice(-1)]
	...
	const nextStepURL = [
		...path.slice(0, onStep1 ? -1 : -2), form.getAttribute('action'), ...path.slice(-1)
	].join("/")
	...
	form.addEventListener('submit', function(event) {
		...
		setTimeout(() => window.location = nextStepURL, 350);
	}, { once:true });
